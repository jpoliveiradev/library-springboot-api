...
public class RentalSpecification {
    public static Specification<Rental> searchSpecification(String search) {
        return (root, query, criteriaBuilder) -> {
            ...
            Join<Rental, Book> bookJoin = root.join("book", JoinType.LEFT);
            Join<Rental, Customer> customerJoin = root.join("customer", JoinType.LEFT);
            Join<Rental, RentalStatus> rentalStatuJoin = root.join("status", JoinType.LEFT);
            ...
            Predicate statusNamePredicate = criteriaBuilder.like(criteriaBuilder.lower(criteriaBuilder.function("unaccent", String.class, rentalStatuJoin.get("name"))), likeSearch);
            ...
            return criteriaBuilder.or(idPredicate, customerNamePredicate, bookNamePredicate, statusNamePredicate, rentalDatePredicate, forecastDatePredicate, returnDatePredicate);
        };
    }
}
